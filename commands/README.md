...
- Writes/refines **`reopenspec.project.yaml`** (work-item integration hints, `changes/` root, traceability paths)
...
- After you type **`approved`**, creates **`changes/active/<change-domain-id>/`**: `plan.md`, `design.md`, `tasks.md`, `delta.md`, `meta.json`

## 4 — Implement that change

**`/reo-proceed-plan @changes/active/<change-domain-id>`**

Runs through **`tasks.md`**, writes **`implementation.md`**, then commit after approval.

## 5 — Complete (human-run; confirm spec sync)

After you verify proceed-plan work is done, run **`/reo-completed @changes/active/<id>`**.

- The agent **proposes** updates to **`specs/`** — you **confirm** before any write.
- Then you **confirm** again (or once, if you asked for a combined step) before moving the folder to **`changes/completed/…`**.
...
| `reo-plan.md` | `/reo-plan` | Work item + delta → `changes/active/…/` |
...
| `reo-proceed-plan.md` | `/reo-proceed-plan` | Implement a `changes/active/…/` folder |
| `reo-completed.md` | `/reo-completed` | Propose **`specs/`** sync + archive — **user confirms** before writes / move |
